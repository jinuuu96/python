"""
input 함수 : 키보드를 입력받을때부터 시작하는 함수를 기억한다.

name = input('이름을 입력하세요>>')
age = input('나이를 입력하세요>>')

@print 함수 : 출력 받을수 있는 함수
print("입력된 이름은{0}나이는{1}".format(name,age))
"""

"""
문제1 : 50000원짜리 물건을 할부로 구입한 뒤 매달 내는 돈을 계산해서 보여줄 프로그램을 만들어 보자
price = 50000
print("물건의 가격 : {0}".format(price))
n = int(input("할부 개월 입력>>"))
print("매달 내는 돈은{0}원 입니다.".format(price/n))


문제2 : 사용자로부터 1에서 12사이에 월을 입력받아 해당 월이 며칠까지 있는지 출력하는 프로그램을 만들어 보자.
(리스트: 배열 활용한 문제)
days = [31,28,31,30,31,30,31,31,30,31,30,31]
month = int(input("1~12사이의 월을 입력하세요>>"))
day = days[month-1]
print("{0}월은 {1}".format(month,day))
"""
"""
문제3 : 영어사전을 구현하고자 한다. 딕셔너리를 하나 생성하고 예시와 같이 동작하자.
...
사원번호의 끝자리 숫자가 5 이상이면 오전, 아니면 오후를 출력하자.


4자리 사원번호를 입력하세요>>1244
근무시간은 오전입니다.
...
문제4-5(교과는 86번) 국어 영어 수학 점수를 각각 입력받아 평균을 구하고
평균이 80점 이상이면 합격, 아니면 불합격을 출력한다.

국어 점수를 입력하세요>>
영어 점수를 입력하세요>>
수학 점수를 입력하세요>>
평균은 83.0 이고 결과는 합격/불합격입니다.
...
id = input("아이디를 입력하세요:>>")
password = input("비밀번호을 입력하세요:>>")
level = int(input("레벨을 입력하세요:>>"))
if id=='admin' and password==123456 and level==1:
    print("관리자입니다.")
else:
    print("회원입니다.")
...
2. 입력하는 월이 6과 같거나 크면서 8와 같거나 작으면 여름이다.
2. 입력하는 월이 9과 같거나 크면서 11와 같거나 작으면 가을이다.
2. 입력하는 월이 12과 같거나 크면서 2와 같거나 작으면 겨울이다.
...
seat = int(input("좌석의 종류를 입력하세요(1.일반실,2:특실):>>"))
...
문제(교과99페이지) 3번문제 : 
임의의 정수를 3개를 입력받아서 그중에서 가장 큰 수를 출력하는 프로그램을 만들어보자
정수1 입력:>>3
정수2 입력:>>1
정수3 입력:>>2
가장 큰 수는 3입니다.
